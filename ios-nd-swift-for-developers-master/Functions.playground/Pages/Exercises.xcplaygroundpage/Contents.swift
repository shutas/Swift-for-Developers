...
func combineLastCharacters(_ stringArray: [String]) -> String {
    var newString = ""
    for string in stringArray {
        newString.append(string[string.index(before: string.endIndex)])
    }
    return newString
}
...
func isDigits(_ string: String) -> Bool {
...
func purgeArray(_ dirtyWord: [String]) -> [String] {
    var cleanArray = [String]()
    for string in dirtyWord {
        if string.characters.count != 4 {
            cleanArray.append(string)
        }
    }
    return cleanArray
}
...
class MovieArchive {
    func filterByDirector(dictionary: Dictionary<String, String>, theDirector: String) -> [String] {
        var titleArray = [String]()
        for (title, director) in dictionary {
...
var a = MovieArchive()
print(a.filterByDirector(dictionary: movies, theDirector: "Christopher Nolan"))
//: [Next](@next)
